...
class MyAgent(Agent):
    ...
    def max_value(self, state, alpha, beta, depth, step, towers):        
        infinity = math.inf
        
        if state.is_finished():
            return self.heuristic(state, towers), None
        if depth > self.get_max_depth(state):
            return self.heuristic(state, towers), None
        v, move = -infinity, None
        for a in self.newActions(state, step, towers):
            v2, _ = self.min_value(state.clone().play_action(a), alpha, beta, depth+1, step, towers)
            if v2 > v:
                v, move = v2, a
                alpha = max(alpha, v)
            if v >= beta:
    ...
    def min_value(self, state, alpha, beta, depth, step, towers):
        infinity = math.inf
    ...
    def newActions(self, board, step, towers):
        if(step < 10):
            for tower in towers:
